...
import { ArrowRight, CheckCircle2, RefreshCw, Sparkles, X, Sunrise, PenLine, GripVertical, Pencil, Check, RotateCcw, Eye, EyeOff } from 'lucide-react';
...
export default function Main() {
  ...
  const widgets: Record<MainWidgetId, ReactNode> = {
    recap: <YesterdayRecapCard habits={habits} />,
    ...
    garden: (
      <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card p-4 space-y-2 flex-1 flex flex-col">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => navigate('/garden')} className="flex items-center gap-1 text-xs text-[var(--leaf)]">
            정원 가기 <ArrowRight size={13} />
          </button>
        </div>
        <div className="relative flex-1 flex items-end justify-center gap-2 rounded-[var(--radius)] bg-gradient-to-b from-[var(--garden-sky-top)] via-[var(--garden-sky-bottom)] to-[var(--leaf-soft)] py-4 min-h-[140px] overflow-hidden">
          <div className="pointer-events-none absolute -top-6 right-1 z-0 h-20 w-20 rounded-full" style={{ background: 'radial-gradient(circle, var(--garden-sun) 0%, transparent 70%)', opacity: 0.65 }} />
          <div className="pointer-events-none absolute inset-x-0 bottom-0 z-0 h-9 rounded-b-[var(--radius)]" style={{ background: 'linear-gradient(to top, var(--garden-soil-bottom) 0%, var(--garden-soil-top) 55%, transparent 100%)', opacity: 0.45 }} />
          {plants.length === 0 ? (
            <p className="relative z-10 self-center text-sm text-[var(--fg-faint)]">씨앗을 심어보세요 🌱</p>
          ) : (
            plants.slice(0, 4).map((p) => (
              <PlantSVG key={p.id} speciesId={p.speciesId} stage={p.stage} withered={!!p.witheredSince} rarity={PLANT_SPECIES.find((sp) => sp.id === p.speciesId)?.rarity} size={72} className="relative z-10" />
            ))
          )}
        </div>
        <div className="flex items-center justify-between">
    ...
    morningBrief: dayDoc?.morningBrief ? (
      <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} className="rounded-[var(--radius)] border border-[var(--bloom)]/20 bg-gradient-to-br from-[#FFF6E5] to-[#FFE9C2] p-3">
        <div className="flex items-center gap-1.5 text-xs font-semibold text-[var(--bloom)]"><Sunrise size={14} /> 오늘의 브리프</div>
        ...
        <div className="mt-1.5 flex flex-wrap gap-1.5">
          {dayDoc.morningBrief.priorityHabits.map((h) => (
            <span key={h.id} className="rounded-full bg-white/70 px-2 py-0.5 text-[11px] text-[var(--fg-primary)]">⭐ {h.title}</span>
          ))}
        </div>
      </motion.div>
    ) : null,
    ...
    weeklyQuest: <WeeklyQuestCard />,

    season: <SeasonCard />,

    oneYearAgo: <OneYearAgoCard />,

    comeback: comeback.active ? (
      <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} className="relative flex items-start gap-3 rounded-[var(--radius)] border border-[var(--bloom)]/30 bg-[var(--bloom-soft)] p-3">
        ...
        <button onClick={comeback.dismiss} aria-label="닫기" className="-mt-1 -mr-1 rounded-full p-1 text-[var(--fg-faint)] hover:text-[var(--fg-muted)]">
          <X size={14} />
        </button>
      </motion.div>
    ) : null,
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-1.5">
                <div className="h-1.5 w-24 overflow-hidden rounded-full bg-white/25">
                  <div className="h-full rounded-full bg-white transition-all" style={{ width: `${Math.min((xpInLevel / xpNeeded) * 100, 100)}%` }} />
                </div>
      ...
      {editMode ? (
        ...
        <>
          {draftOrder.map((id) => {
            ...
            if (!node) return null;
            return (
              <div key={id} className={cn('flex flex-col', id === 'garden' && 'flex-1')}>
                {node}
              </div>
            );
          })}
