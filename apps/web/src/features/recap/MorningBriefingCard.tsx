...
/**
 * 아침 브리핑 — '어제 돌아보기'와 '오늘의 브리프'를 하나로 합친 메인 상단 카드.
 *
 *  1. 어제 회고에 적은 '내일의 다짐(q_tomorrow)'을 오늘 가장 먼저 보여주고,
 *     실천 체크로 전날 피드백이 오늘 행동으로 이어지도록 장려한다.
...
export default function MorningBriefingCard({
  ...
  return (
    <motion.section
      ...
      className="relative space-y-2.5 rounded-[var(--radius)] border border-[var(--bloom)]/25 bg-[var(--bg-surface)] p-3.5 shadow-[var(--shadow-sm)]"
    >
      ...
      {allDone ? (
        ...
        <>
          ...
          {/* 오늘의 개선 포인트 — 가장 중요한 놓친 습관 하나 (다짐이 없을 때만 — 중복 강조 방지) */}
