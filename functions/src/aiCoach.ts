...
export type Mode = 'daily' | 'crisis' | 'weekly';
...
export const aiCoach = functions
  ...
  .onCall(async (data, context) => {
    ...
    return runAICoach(uid, (data?.mode ?? 'daily') as Mode);
  });

/**
 * 코치 응답 생성 본체 — callable(aiCoach)과 텔레그램 봇(/coach, /weekly)이 공유한다.
 * 캐시(users/{uid}/coach/{date}_{mode})도 공유하므로 앱과 봇이 같은 응답을 본다.
 * 호출부가 인증·승인 게이트를 책임진다.
 */
export async function runAICoach(uid: string, mode: Mode): Promise<Record<string, any>> {
  ...
  if (mode !== 'crisis') {
    ...
    if (cached.exists) return cached.data() as Record<string, any>;
  }
