...
> 건너뛰기·휴면·보호된 날(휴가/아픔/그레이스)은 패널티 제외. 사용 포인트만 차감하고 레벨·누적 XP는 보존(레벨 후퇴 없음). **더보기 → 설정 → 습관 미완료 패널티** 토글로 끌 수 있음(기본 ON).

## 주요 기능 메모

- **습관 묶음 · 일괄 건너뛰기**: 습관 편집에서 '학교' 같은 묶음을 만들어 습관을 배정하면, 습관 화면 상단에서 묶음 단위로 **오늘 일괄 건너뛰기/해제** 가능. 등교 안 하는 날 학교 습관을 한 번에 건너뛴다.
- **말씀 적용 추적** (신앙 기능): 큐티·주일설교·말씀묵상에서 받은 *적용*(무엇을 실천할지)을 기록하고, 이후 며칠간 "오늘 실천했어요"를 체크해 실천 횟수·연속일을 추적한다. 목표일 달성 시 완료로 마무리(+20P). 더보기 → **말씀 적용**.
