...
빠른 체크는 한 화면에 습관을 6개까지만, 습관당 버튼 하나로 표시한다. 이진 습관은
누르면 바로 완료되고 점수형 습관은 1~5점 선택 화면으로 들어간다. `시간대`를 누르면
아침·오후·저녁·밤·언제든·전체 선택 화면이, `수정·건너뜀`을 누르면 보조 기록 화면이
따로 열린다. `나중에 알림` 안의 `30분 뒤`·`2시간 뒤`는 서버에 재알림을 예약하며,
`오늘 알림 끝`은 다음 날 자동 복구되는 당일 중지다.
