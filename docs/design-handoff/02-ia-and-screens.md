...
> **초보자 레이아웃은 조건부다.** `firstRunSetup`은 온보딩을 끝낼 때 **습관이 하나도 없던 사용자에게만**
> 코어 3개(`BEGINNER_CORE_WIDGETS`)를 남기고 나머지를 숨긴다. 게스트는 로그인 시 `seedGuestHabits`로
> 습관 3개를 먼저 받으므로 이 조건에 걸리지 않고 **위젯이 전부 보인 채로 시작한다**
> ([`10-main`](screens/10-main.png) 캡처가 그 상태다). 즉 첫 화면 밀도는 가입 경로에 따라 다르다.
